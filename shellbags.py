...
def print_shellbag_csv(shellbags, regfile, output):
    csv_file = open(output, "w")
    stdoutWriter = csv.writer(csv_file)
                        
                    
                    
    for shellbag in shellbags:
        keymod = datetime.datetime.strptime(date_safe_str(shellbag["klwt"]), "%Y-%m-%d %H:%M:%S")
        time_gap = datetime.timedelta(hours=9)
        strtime = keymod + time_gap
        try:
            stdoutWriter.writerow([strtime,shellbag["path"].encode('euc-kr')])
        except:
            stdoutWriter.writerow([strtime,   "Unprintable Shellbag",
                                   ])
...
def main(fname, fmt, output):
    registry = Registry.Registry(fname)

    parsed_shellbags = get_all_shellbags(registry)

    if fmt == "csv":
         print_shellbag_csv(parsed_shellbags, fname, output)
    elif fmt == "bodyfile":
...
def shellbag_main(fname, fmt, output):

    main(fname, fmt, output)
